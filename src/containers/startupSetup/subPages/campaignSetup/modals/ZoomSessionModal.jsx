...
function ZoomSessionModal({ details, onSubmitFunc }){
    const [showModal, setShowModal] = useState(false);
    const [temporaryDetails, setTemporaryDetails] = useState({
        date: details.date,
    ...
    function changeTemporaryDetails(key, value){
        setTemporaryDetails(prevState => ({
            ...prevState,
            [key]: value
        }))
    }
                                ...
                                <div className="p-6 flex flex-wrap align-middle justify-center ">
                                    ...
                                    <div className="flex flex-wrap flex-col w-full">
                                        <div className="flex flex-row justify-center items-center">
                                            ...
                                            <PrimaryInput placeholder="dd/mm/yy" properties="text-center w-1/2 sm:w-1/3 text-xs md:text-md" onChange={(e) => changeTemporaryDetails("date", e.target.value)}
                                                          value={temporaryDetails.date} type="date" />
                                        </div>
                                        <div className="flex flex-row justify-center items-center">
                                            ...
                                            <div className="w-1/2 items-stretch sm:w-1/3 flex flex-col sm:flex-row justify-between sm:items-center m-4">
                                                <input className="rounded-xl bg-gray-100 placeholder-gray-400 font-Inter text-center py-2 text-xs sm:text-base"
                                                       placeholder="0000" onChange={(e) => changeTemporaryDetails("startTime", e.target.value)}
                                                       value={temporaryDetails.startTime} type="time" />
                                                <p className="text-center text-xs md:text-base font-bold">TO</p>
                                                <input className="rounded-xl bg-gray-100 placeholder-gray-400 font-Inter text-center py-2 text-xs sm:text-base"
                                                       placeholder="2359" onChange={(e) => changeTemporaryDetails("endTime", e.target.value)}
                                                       value={temporaryDetails.endTime} type="time" />
                                            </div>
                                        </div>
                                        <br />
                                        <br />
                                        <PrimaryButton properties="self-end" text="Submit" onClick={ () => { onSubmitFunc(temporaryDetails); ModalFunc() } }/>
                                    </div>
                                </div>
                            </div>
                        </div>
